...
from model.plstm import PhasedLSTM, FoldPhasedLSTM
...
def run(dataset_name, model_name):
    ...
    for rnn_num, rnn_dim, dnn_num in itertools.product(rnn_nums, rnn_dims, dnn_nums):
        ...
        model = {'sim': SimpleLSTM, 'pha': PhasedLSTM, 'att': Attention}[model_name]
...
def run_fold(dataset_name, model_name, hyper_param):
    ...
    for fold in map(str, range(0, 10)):
        fold_model = {'sim': FoldSimpleLSTM, 'pha': FoldPhasedLSTM, 'att': FoldAttention}[model_name]
