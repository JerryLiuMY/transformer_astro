...
def std(dataset_name='OGLE'):
    ...
    catalog = pd.read_csv(os.path.join(RAW_FOLDER, dataset_name, 'catalog.csv'))
    base = os.path.join(RAW_FOLDER, dataset_name, 'LCs')
...
def scaler_saver(dataset_name, feature_range=(0, 30)):
    ...
    for cat, path in tqdm(list(zip(cats, paths))):
        data_df = pd.read_pickle(os.path.join(RAW_FOLDER, dataset_name, path))
        mag_full = np.append(mag_full, np.array(data_df['mag']))
    scaler = MinMaxScaler(feature_range=feature_range)
    scaler.fit(mag_full.reshape(-1, 1))

    with open(os.path.join(RAW_FOLDER, dataset_name, 'scaler.pkl'), 'wb') as handle:
